...
import { ScorePenaltyEngine, type ScorePenaltyConfig } from './score-penalty-engine';

describe('ScorePenaltyEngine - Property Tests', () => {
  /**
    ...
    it('should implement first-retry-free policy correctly', () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 100, max: 5000 }), // originalScore
          (originalScore) => {
            const engine = new ScorePenaltyEngine({ firstRetryFree: true });
    ...
    it('should use logarithmic penalty curve', () => {
      fc.assert(
        fc.property(
          ...
          (retryCount) => {
            const engine = new ScorePenaltyEngine({ penaltyType: 'logarithmic' });
